...
//                "    }\n" +
    ...
    public static List<String> generateAll(Table table, String packageName, String baseJar) {
        String modelName = StringFormatUtils.snake(table.name, true);
        List<String> ans = new ArrayList<>();
        String file1 = FileUtils.createFileWithRelativePath("generate/model", modelName + ".java", getModelStr(table));
        String file2 = FileUtils.createFileWithRelativePath("generate/query", modelName + "Query.java", getQueryStr(table, packageName, baseJar));
        String file3 = FileUtils.createFileWithRelativePath("generate/repository", modelName + "Repository.java", getRepositoryStr(table, packageName, baseJar));
        String file4 = FileUtils.createFileWithRelativePath("generate/impl", modelName + "RepositoryImpl.java", getImplStr(table, packageName, baseJar));
        ans.add(file1);
        ans.add(file2);
        ans.add(file3);
        ans.add(file4);
    ...
    public static void generateOrmClass(String fileName, String packageName, String baseJar) {
        List<Table> tables = getTableStrs(fileName);
        List<String> files = new ArrayList<>();
        for (Table table : tables) {
            files.addAll(generateAll(table, packageName, baseJar));
        }
    }
